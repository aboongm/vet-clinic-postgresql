...
SELECT name, full_name FROM animals FULL JOIN owners ON animals.owners_id = owners.id;

/* How many animals are there per species */
SELECT species.name, COUNT(*) FROM animals FULL JOIN species ON animals.species_id = species.id GROUP BY species.name;

/* Digimon owned by Jennifer Orwell. */
SELECT species.name, animals.name, owners.full_name FROM animals FULL JOIN owners ON animals.owners_id = owners.id
...
SELECT animals.name, animals.escape_attempts FROM animals FULL JOIN owners ON animals.owners_id = owners.id
...
JOIN vets ON vets.id = visits.vets_id WHERE vets_id = 1 ORDER BY date_of_visit DESC LIMIT 1;
...
JOIN vets ON vets.id = visits.vets_id WHERE vets_id = 3;

/* List all vets and their specialties, including vets with no specialties */
SELECT vets.name, species.name FROM vets FULL JOIN specializations ON vets.id = specializations.vets_id
FULL JOIN species ON species.id = specializations.species_id;

/* List all animals that visited Stephanie Mendez between April 1st and August 30th, 2020 */
SELECT animals.name, visits.date_of_visit FROM animals FULL JOIN visits ON animals.id = visits.animals_id
FULL JOIN vets ON vets.id = visits.vets_id WHERE vets_id = 3 AND date_of_visit BETWEEN '2020-04-01' AND '2020-08-30';

/* What animal has the most visits to vets? */
SELECT animals.name, COUNT(*) as visited FROM animals FULL JOIN visits ON animals.id = visits.animals_id
FULL JOIN vets ON vets.id = visits.vets_id GROUP BY animals.name ORDER BY visited DESC LIMIT 1;

/* Who was Maisy Smith's first visit? */
SELECT animals.name, visits.date_of_visit FROM animals FULL JOIN visits ON animals.id = visits.animals_id
FULL JOIN vets ON vets_id = visits.vets_id WHERE vets_id = 2 ORDER BY visits.date_of_visit LIMIT 1;
...
WHERE vets_id = 2 GROUP BY species.name;
